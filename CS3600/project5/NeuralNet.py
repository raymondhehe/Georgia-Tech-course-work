...
class Perceptron(object):
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for i in range(len(acts)):
            tmp = acts[i] * alpha * delta
            self.weights[i] += tmp
            totalModification += abs(tmp)
...
class NeuralNet(object):                                    
    ...
    def feedForward(self, inActs):
        ...
        for l in self.layers:
            tmpList = []
            for p in l:
                tmpList.append(p.sigmoidActivation(mylist[len(mylist) - 1]))
    ...
    def backPropLearning(self, examples, alpha):
        ...
        for example in examples:#for each example
            ...
            for outputNum in xrange(len(example[1])):
                gPrime = self.outputLayer[outputNum].sigmoidActivationDeriv(allLayerOutput[-2]) #edit
                error = example[1][outputNum] - lastLayerOutput[outputNum] #edit
                delta = error * gPrime #edit
            ...
            for layerNum in xrange(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in xrange(len(layer)):
                    gPrime = layer[neuronNum].sigmoidActivationDeriv(allLayerOutput[layerNum]) #edit
                    nextE = 0
                    for nextN in xrange(len(nextLayer)):
                        wDelta = nextLayer[nextN].weights[neuronNum + 1] * deltas[0][nextN] #edit
                        nextE += wDelta #edit
                    delta = gPrime * nextE #edit
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxint, startNNet = None):
    ...
    testError = 0.0
    testCorrect = 0.0
    for eIn, eOut in examplesTest:
        tmpList = nnet.feedForward(eIn)[-1]
        check = True
        for val in range(len(tmpList)):
            if round(tmpList[val]) != eOut[val]:
                check = False
        if check:
            testCorrect += 1.0
        else:
