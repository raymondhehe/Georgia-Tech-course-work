...
def getNNXorData(fileString = "datasets/xor.data.txt", limit = 100000): #q7
    examples=[]
    attrValues={}
    data = open(fileString)
    attrs = ['x','y']
...
def buildExamplesFromXorData(size = 4): #q7
    ...
    for cdRec in xorData:
        ...
        for cdInRec in cdRec[0]:
            for val in cdInRec:
                tempInVec.append(val)
		#print "in :" + str(cdRec) + " in vec : " + str(tmpInVec)
        tempList = (tempInVec, cdRec[1])
        xorDataTrainList.append(tempList)
	#print "car data list : " + str(carDataList)
