...
def consistent(assignment, csp, var, value):
	...
	for c in csp.binaryConstraints:
		if c.affects(var):
			if assignment.isAssigned(c.otherVariable(var)):
				if not c.isSatisfied(value, assignment.assignedValues[c.otherVariable(var)]):
					return False
	return True
...
def recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod):
	...
	for v in orderValuesMethod(assignment, csp, var):
		if consistent(assignment, csp, var, v):
			assignment.assignedValues[var] = v
...
def minimumRemainingValuesHeuristic(assignment, csp):
	...
	for var in domains:
		if not assignment.isAssigned(var):
			currentDegree = 0
			for c in csp.binaryConstraints:
				if c.affects(var) and not assignment.isAssigned(var):
					currentDegree = currentDegree + 1
			currentMRV = len(domains[var])
			if (currentMRV < mrv) or (currentMRV == mrv and currentDegree > degree): #case 1: mrv; case2: tie-breaker
				mrv = currentMRV
				nextVar = var
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
	...
	for c in csp.binaryConstraints:
		if c.affects(var):
			affectedList.append(c.otherVariable(var))
	for v in values:
		count = 0
		for a in affectedList:
			if v in list(assignment.varDomains[a]):
				count = count + 1
		valuesList.append((v, count))
	valuesList.sort(key=lambda col: col[1])
	for v in valuesList:
		returnList.append(v[0])
	return returnList
"""
...
def forwardChecking(assignment, csp, var, value):
	...
	for c in csp.binaryConstraints:
		if c.affects(var):
			...
			if value in domains[v]:
				domains[v].remove(value)
				inferences.add((v, value))
			if len(domains[v]) == 0:
				for i in inferences:
					domains[i[0]].add(i[1])
				return None
...
def recursiveBacktrackingWithInferences(assignment, csp, orderValuesMethod, selectVariableMethod, inferenceMethod):
	...
	for v in orderValuesMethod(assignment, csp, var):
		if consistent(assignment, csp, var, v):
			assignment.assignedValues[var] = v
			inferences = inferenceMethod(assignment, csp, var, v)
			if inferences != None:
				result = recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod)
				if result != None:
					return assignment
...
def revise(assignment, csp, var1, var2, constraint):
	...
	"""Question 5"""
	"""YOUR CODE HERE"""
	for x in domains[var2]:
		check = False
		for y in domains[var1]:
...
def maintainArcConsistency(assignment, csp, var, value):
	...
	for c in csp.binaryConstraints:
		if c.affects(var):
			q.put((var, c.otherVariable(var), c))
	while not q.empty():
		element = q.get()
		r = revise(assignment, csp, element[0], element[1], element[2])
		if r == None:
			for v in inferences:
				assignment.varDomains[v[0]].add(v[1])
			return None
		if len(r) != 0:
			for c in csp.binaryConstraints:
				if c.affects(element[1]):
					q.put((element[1], c.otherVariable(element[1]), c))
			inferences = inferences.union(r)
	return inferences
