...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i,beliefs
                                            ...
                                            if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        maxProb, ghostPos = 0.0, None
        for ghost in livingGhostPositionDistributions:
            for pos, prob in ghost.items():
                if prob > maxProb: maxProb, ghostPos = prob, pos

        Dist = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, legal[0]), ghostPos)
        result = legal[0]
        for l in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, l)
            if self.distancer.getDistance(successorPosition, ghostPos) < Dist:
