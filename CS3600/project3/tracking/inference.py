...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            totalWeight = allPossible.totalCount()
            if totalWeight == 0:
                self.initializeUniformly(gameState)
            else:
                for r in range(0, self.numParticles):
                    pList.append(util.sample(allPossible))
                self.particlesList = pList
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particlesList:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            pList.append(util.sample(newPosDist))
        self.particlesList = pList
        #util.raiseNotDefined()
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        while len(self.particles) < self.numParticles:
            for p in product(self.legalPositions, repeat = self.numGhosts):
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        allPossible = util.Counter()
        ghostList = []
        for r in range(self.numGhosts):
            if noisyDistances[r] is None: ghostList.append(r)

        for p in self.particles:
            for g in ghostList: p = self.getParticleWithGhostInJail(p, g)
            pWeight = 1
            for r in range(self.numGhosts):
                if r not in ghostList:
                    trueDistance = util.manhattanDistance(p[r], pacmanPosition)
                    pWeight = pWeight * emissionModels[r][trueDistance]
            allPossible[p] = allPossible[p] + pWeight
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            for i in range(0, self.numGhosts):
                newDist = getPositionDistributionForGhost(setGhostPositions(gameState, oldParticle), i, self.ghostAgents[i])
